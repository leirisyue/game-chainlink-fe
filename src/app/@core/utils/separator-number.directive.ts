// https://stackblitz.com/edit/ngx-number-input-3nigpx?file=src%2Fapp%2Fapp.component.ts,src%2Fapp%2Fapp.component.html,src%2Fapp%2Fnumber-input.directive.ts
...
import * as numeral from 'numeral';
  ...
  class NumberBuilder {
    ...
    limit(decimals: number): NumberBuilder {
      ...
      if (decimalPart && decimalPart.length > decimals) {
        return new NumberBuilder(parseFloat(`${firstPart}.${decimalPart.substring(0, decimals)}`));
      }
  ...
  @Directive({
    selector: 'input[separatorNumber]',
    providers: [
      {
        provide: NG_VALUE_ACCESSOR,
        useExisting: forwardRef(() => SeparatorNumberDirective),
        multi: true
      }
    ],
    exportAs: 'separatorNumber'
  })
  export class SeparatorNumberDirective implements ControlValueAccessor, OnChanges {
